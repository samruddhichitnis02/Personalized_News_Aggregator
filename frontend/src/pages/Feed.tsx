...
const Feed = () => {
  ...
  }, [mode, visibleCount, articles.length]);
  ...
  const getReadMins = (a: Article) => {
    const words = [(a.title || ''), (a.description || '')].join(' ').trim().split(/\s+/).filter(Boolean).length;
  ...
  const filtered = articles
    ...
    .filter(a => {
      ...
      const mins = getReadMins(a);
      if (readFilter === 'quick') return mins <= 2;
      if (readFilter === 'deep') return mins >= 4;
      return true;
    });
      ...
      <main style={{ maxWidth: '1280px', margin: '0 auto', padding: '40px 40px 80px' }}>
        ...
        <div style={{
          ...
          {mode === 'feed' && (
            <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
              {([
                { key: 'all',   label: 'All reads',    icon: '⏱' },
                { key: 'quick', label: 'Quick ≤2 min', icon: '⚡' },
                { key: 'deep',  label: 'Deep ≥4 min',  icon: '📖' },
              ] as const).map(({ key, label, icon }) => (
                <button
                  key={key}
                  onClick={() => { setReadFilter(key); setVisibleCount(9); }}
                  style={{
                    backgroundColor: readFilter === key ? 'var(--accent)' : 'var(--bg-card)',
                    border: `1px solid ${readFilter === key ? 'var(--accent)' : 'var(--border-light)'}`,
                    borderRadius: '100px', padding: '5px 14px',
                    fontSize: '0.74rem', fontWeight: 600,
                    color: readFilter === key ? '#080808' : 'var(--text-secondary)',
                    cursor: 'pointer', transition: 'all 0.18s ease',
                    fontFamily: 'var(--font-body)',
        ...
        {!isLoading && (
          <>
            ...
            {mode === 'feed' && (
              <div ref={sentinelRef} style={{ height: '48px', marginTop: '32px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {hasMore && (
                  <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                    {[0, 1, 2].map(i => (
                      <div key={i} style={{
                ...
                {!hasMore && filtered.length > 1 && (
                  <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', letterSpacing: '0.04em' }}>
                    ✦ You've reached the end · {filtered.length} stories
                  </p>
                )}
              </div>
            )}
